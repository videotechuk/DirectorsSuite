...
{
	// When an external owner (Photo Mode) drives input, skip our own reads so
	// its camera keys never leak into menu navigation and vice-versa.
	if (!m_InputSuspended) {
		CheckInput();
		HandleInput();
	}
	...
	bool promptsVisible = m_IsOpen && !m_ForceHidden;
	HUD::_UI_PROMPT_SET_VISIBLE(m_SelectPrompt, promptsVisible);
	HUD::_UI_PROMPT_SET_ENABLED(m_SelectPrompt, promptsVisible);
	HUD::_UI_PROMPT_SET_VISIBLE(m_BackPrompt, promptsVisible);
	HUD::_UI_PROMPT_SET_ENABLED(m_BackPrompt, promptsVisible);
}


void CNativeMenu::HidePrompts()
{
	HUD::_UI_PROMPT_SET_VISIBLE(m_SelectPrompt, false);
	HUD::_UI_PROMPT_SET_ENABLED(m_SelectPrompt, false);
	HUD::_UI_PROMPT_SET_VISIBLE(m_BackPrompt, false);
	HUD::_UI_PROMPT_SET_ENABLED(m_BackPrompt, false);
}
...
void CNativeMenu::CheckInput()
{
	...
	m_OpenKeyPressed	= IsKeyJustUp((DWORD)g_Config.KeyOpenMenu) || PAD::IS_DISABLED_CONTROL_PRESSED(2, INPUT_FRONTEND_RB) && PAD::IS_CONTROL_JUST_RELEASED(2, INPUT_CONTEXT_A);
	m_EnterKeyPressed	= PAD::IS_CONTROL_JUST_RELEASED(m_ControlIndex, INPUT_GAME_MENU_ACCEPT);
	m_BackKeyPressed	= PAD::IS_CONTROL_JUST_RELEASED(m_ControlIndex, INPUT_GAME_MENU_CANCEL) || IsKeyJustUp(VK_BACK);
...
void CNativeMenu::HandleEnterPressed()
{
	...
	if (!m_SuppressNavSound) playSoundFrontend("SELECT", "HUD_SHOP_SOUNDSET");
}
...
void CNativeMenu::HandleBackPressed()
{
	if (m_CurrentSubmenu->ID <= eSubmenuID::Submenu_EntryMenu) {
		if (!m_SuppressNavSound) playSoundFrontend("BACK", "HUD_SHOP_SOUNDSET");
		SetEnabled(false);
		return;
	}
...
// Force the menu open at a specific submenu with a clean back-stack, so an
// external owner (Photo Mode) can host its own page tree.
void CNativeMenu::OpenAt(eSubmenuID id)
{
	if (!m_SubmenusMap.contains(id)) {
		PRINT_ERROR("OpenAt: submenu ID ", (int)id, " doesn't exist");
		return;
	}
	m_PreviousSubmenus.clear();
	m_LastSubmenuSelections.clear();
	m_CurrentSubmenu = &m_SubmenusMap[id];
	m_CurrentSubmenuID = id;
	m_SelectionIndex = 0;
	m_IsOpen = true;
}


// Back one page; false if already at `root` (caller decides what to do, e.g.
// exit Photo Mode).
bool CNativeMenu::ExtBack(eSubmenuID root)
{
	if (m_CurrentSubmenu == nullptr || m_CurrentSubmenu->ID == root) {
		return false;
	}
	HandleBackPressed();
	return true;
...
void CNativeMenu::HandleUpKeyPressed()
{
	...
	// Skip over consecutive empty options (separators and help text rows)
	for (int guard = 0; guard < numOptions && GetSelectedOption()->IsEmptyOption; guard++) {
		m_SelectionIndex--;
		if (m_SelectionIndex < 0) { m_SelectionIndex = numOptions - 1; }
	}

	if (!m_SuppressNavSound) playSoundFrontend("NAV_UP", "Ledger_Sounds");
}
...
void CNativeMenu::HandleDownKeyPressed()
{
	...
	if (!m_SuppressNavSound) playSoundFrontend("NAV_DOWN", "Ledger_Sounds");
}
...
void CNativeMenu::HandleLeftKeyPressed()
{
	...
	if (option->IsVectorOption) {
		...
		if (!m_SuppressNavSound) playSoundFrontend("NAV_LEFT", "PAUSE_MENU_SOUNDSET");
	}
}
...
void CNativeMenu::HandleRightKeyPressed()
{
	...
	if (option->IsVectorOption) {
		...
		if (!m_SuppressNavSound) playSoundFrontend("NAV_RIGHT", "PAUSE_MENU_SOUNDSET");
	}
}
