...
// Y increment between option rows, in 1080p design-space pixels.
// Every other layout constant in this file (option start, footer, scroller)
// is a fixed 1080p pixel value that gets normalized by the real screen size,
// so the row increment must be a fixed design value too. The old version
// used SCREEN_HEIGHT * 0.05 (real pixels), which pushed the last rows down
// into the footer text on any display taller than 1080p.
const float INCREMENT = 54.0f;
#pragma endregion
...
inline void DrawFooter(const std::string& text)
{
	...
	// Word-wrap long descriptions so they stay inside the menu width instead
	// of spilling out both sides of the background.
	constexpr size_t maxCharsPerLine = 56;
	std::vector<std::string> lines;
	{
		std::stringstream ss(text);
		std::string word, current;
		while (ss >> word) {
			if (!current.empty() && current.size() + 1 + word.size() > maxCharsPerLine) {
	...
	constexpr float lineHeight = (float)uiFontSizes::Footer + 7.0f;

	// Descriptions longer than the space left inside the background get a
	// matching backing panel extending below the menu.
	constexpr float bgBottom = uiBackground::PositionY + uiBackground::Height;
	float textBottom = uiFooter::TextPositionY + (float)lines.size() * lineHeight + 6.0f;
	if (textBottom > bgBottom) {
		Drawing::DrawSprite("generic_textures", "inkroller_1a", uiBackground::PositionX, bgBottom - 4.0f,
			uiBackground::Width, (textBottom - bgBottom) + 8.0f, 0, 0, 0, 0, 230, false);
	}

	for (size_t i = 0; i < lines.size(); i++) {
		Drawing::DrawFormattedText(lines[i], Font::Body, COLOR_WHITE_SMOKE, Alignment::Center, uiFontSizes::Footer,
			uiBackground::MiddleX, uiFooter::TextPositionY + (float)i * lineHeight);
	}
}
...
// Empty options come in two flavours:
//  - "--- separator ---" rows (text starts with '-') keep the classic
//    centered title style
//  - anything else is informational text (used by the Help pages): drawn
//    left-aligned in the body font with no button background, so reading
//    content is visually distinct from actionable options.
static void drawEmptyOptionText(Option* pOption, float incr)
{
	if (!pOption->Text.empty() && pOption->Text[0] == '-') {
		Drawing::DrawFormattedText(pOption->Text, Font::Title, COLOR_WHITE_SMOKE, Alignment::Center, uiFontSizes::EmptyOption, uiOption::EmptyOpionTextPositionX, (uiOption::TextPositionYStart - uiFontSizes::EmptyOption) + incr);
	}
	else {
		// Help/info paragraph rows: tighter letter spacing and a cleaner
		// condensed face so the text reads as a paragraph, not spread wide.
		Drawing::DrawFormattedText(pOption->Text, Font::Body, 205, 205, 205, 235, Alignment::Left, 19, uiOption::LeftTextPositionX + 10.0f, (uiOption::TextPositionYStart - uiFontSizes::Option) + incr, 0, -2);
	}
}
...
void drawOptionInternal(Option* pOption, bool drawInRange, bool drawOutOfRange, float inRangeIncr, float outOfRangeIncr)
{
	if (drawInRange)
	{
		if (!pOption->IsEmptyOption) {
			...
			Drawing::DrawFormattedText(pOption->Text, Font::Body, pOption->TextR, pOption->TextG, pOption->TextB, 255, Alignment::Left, uiFontSizes::Option, uiOption::LeftTextPositionX, (uiOption::TextPositionYStart - uiFontSizes::Option) + inRangeIncr);
		}
		else {
			drawEmptyOptionText(pOption, inRangeIncr);
		}
	}
	else if (drawOutOfRange)
	{
		if (!pOption->IsEmptyOption) {
			...
			Drawing::DrawFormattedText(pOption->Text, Font::Body, pOption->TextR, pOption->TextG, pOption->TextB, 255, Alignment::Left, uiFontSizes::Option, uiOption::LeftTextPositionX, (uiOption::TextPositionYStart - uiFontSizes::Option) + outOfRangeIncr);
		}
		else {
			drawEmptyOptionText(pOption, outOfRangeIncr);
		}
	}
}
